...
from probabilityOfDetectionJax import (
    ...
    compute_probability_of_detection_vectorized,
    probability_of_detection_uncertainty_single_radar_at_xy,
    probability_of_detection_uncertainty_single_radar_at_xy_bounds,
)
    ...
    for radar in radarList:
        pdList.append(
            ground_truth_probability_of_detection(
                X_test,
                (radar,),
                params.radarWavelengthPriorMean,
...
def ground_truth_radar_voronoi_weighted(X_test, radarList, ax):
    weights = np.sqrt(
        np.sqrt(
            np.array(
                [
                    radar.outputPower * radar.transmitGain * radar.recieveGain
                    for radar in radarList
                ]
            )
        )
...
def chance_constraint(self, threshold, thresholdConfidence, mean, var):
    ...
    # return (mean - rho) > (erfinv(-2*delta+1)*np.sqrt(2*var))
    return (
        NormalDist(mu=mean, sigma=np.sqrt(var)).cdf(threshold)
    ) > thresholdConfidence


def safe_corridors_uncertain_radar(
    X_test,
    pdThreshold,
    likleyhoodThreshold,
...
def probability_pd_less_than_threshold(
    X_test,
    pdThreshold,
    estimatedRadarParams,
    estimatedRadarParamsCov,
...
def probability_pd_less_than_threshold_single_radar(
    X_test,
    pdThreshold,
    estimatedRadarParam,
    estimatedRadarParamsCov,
    ...
    pdMean = compute_probability_of_detection_vectorized(
        X_test,
        estimatedRadarParam,
        radarRecieveGain,
        radarWavelength,
    ...
    if not useUpperBound:
        pdCov = probability_of_detection_uncertainty_single_radar_at_xy(
            X_test,
            estimatedRadarParam,
            estimatedRadarParamsCov,
            radarRecieveGain,
...
def ground_truth_safe_corridors(X_test, pdThreshold, radarList, ax):
    ...
    ax.pcolormesh(
        X_test[:, 0].reshape(params.numTestPoints, params.numTestPoints),
        X_test[:, 1].reshape(params.numTestPoints, params.numTestPoints),
        safe_corridor.reshape(params.numTestPoints, params.numTestPoints),
    )
    return safe_corridor


def weighted_voronoi_uncertain_radar_grid_method(
    X_test,
    prob_list,
    estimatedRadarParamsList,
    estiamtedRadarParamsCovList,
    useUpperBound=False,
    ax=None,
):
    ...
    for i in range(len(estimatedRadarParamsList)):
        ...
        contour, _ = cv2.findContours(
            Z_temp.astype(np.uint8).T, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
...
def create_probability_of_detection_below_threshold_grid_list(
    X_test, estimatedRadarParamsList, estiamtedRadarParamsCovList, params
):
    prob_list = []

    for i, radar in enumerate(estimatedRadarParamsList):
        prob_list.append(
            probability_pd_less_than_threshold_single_radar(
                X_test,
                params.probabilityOfDetectionThreshold,
                radar,
                estiamtedRadarParamsCovList[i],
                params.radarRecieveGain,
                params.radarRecieveGainPriorVariance,
                params.radarWavelengthPriorMean,
...
def find_contour_points(cellAssignment, index, bounds, numTestPoints):
    ...
    contours, _ = cv2.findContours(
        cellI.reshape(numTestPoints, numTestPoints).astype(np.uint8).T,
        cv2.RETR_TREE,
...
def find_closest_common_point_within_threshold(
    array1, array2, array3, threshold, indexI, indexJ, indexK
):
    closest_points = ()
    ...
    for i in range(len(prob_list)):
        for j in range(i + 1, len(prob_list)):
            for k in range(j + 1, len(prob_list)):
                ...
                commonPointExists, commonPoint = (
                    find_closest_common_point_within_threshold(
                        pointsI,
                        pointsJ,
                        pointsK,
                        2 * bounds[0] / numTestPoints + 100,
                        i,
                        j,
                        k,
                    )
...
def new_filter_points_by_vertices(
    contour,
    vertex1,
    vertex2,
    reference_point,
    ...
    for vertexIndex in allVertices.keys():
        if vertexIndex == vertex1Index or vertexIndex == vertex2Index:
            verticies.append(allVertices[vertexIndex]["point"])
            continue
    ...
    distanceMatrix = np.linalg.norm(
        allOtherVertices - points_in_range[:, np.newaxis], axis=2
    )
    otherDistanceMatrix = np.linalg.norm(
        allOtherVertices - contour[~mask][:, np.newaxis], axis=2
    )

    testDistanceMatrix = np.linalg.norm(
        points_in_range[:, np.newaxis] - verticies, axis=2
    )
    ...
    # ax.plot(contour[:, 0], contour[:, 1], c="g")
    ...
    # ax.scatter(
    #     contour[closestPointToVertex1Index][0],
    #     contour[closestPointToVertex1Index][1],
    #     color="r",
    # )
    # ax.scatter(
    #     contour[closestPointToVertex2Index][0],
    #     contour[closestPointToVertex2Index][1],
    #     color="r",
    # )
    # plt.show()
...
def filter_points_by_vertices(
    ...
    normal = np.array(
        [reference_point[0] - midpoint[0], reference_point[1] - midpoint[1]]
    )
    ...
    angles = np.arctan2(
        contour[:, 1] - reference_point[1], contour[:, 0] - reference_point[0]
    )

    # Calculate angles for the vertices relative to the reference point
    angle_vertex1 = np.arctan2(
        vertex1[1] - reference_point[1], vertex1[0] - reference_point[0]
    )
    angle_vertex2 = np.arctan2(
        vertex2[1] - reference_point[1], vertex2[0] - reference_point[0]
    )
    ...
    if np.linalg.norm(vertex1 - points_in_range[0]) > np.linalg.norm(
        vertex1 - points_in_range[-1]
    ):
...
def move_control_points_so_spline_passes_through_start_and_end(
    ...
    b = np.array(
        [
            [start[0] - (2.0 / 3.0) * c2x - (1.0 / 6.0) * c3x],
            [start[1] - (2.0 / 3.0) * c2y - (1.0 / 6.0) * c3y],
        ]
    )
    ...
    b = np.array(
        [
            [end[0] - (2.0 / 3.0) * cn_minus_1_x - (1.0 / 6.0) * cn_minus_2_x],
            [end[1] - (2.0 / 3.0) * cn_minus_1_y - (1.0 / 6.0) * cn_minus_2_y],
        ]
    )
...
def create_unclamped_knot_points(t0, tf, numControlPoints, splineOrder):
    ...
    knots = np.concatenate(
        (
            np.linspace(t0 - splineOrder * h, t0 - h, splineOrder),
            internalKnots,
            np.linspace(tf + h, tf + splineOrder * h, splineOrder),
        )
    )
...
def fit_spline_to_path(path, num_control_points, spline_order, vertex1, vertex2):
    ...
    combined_control_points = (
        move_control_points_so_spline_passes_through_start_and_end(
            combined_control_points, path[0], path[-1]
        )
    )

    combined_knot_points = tck_x[0]
    combined_knot_points = create_unclamped_knot_points(
        0, tf, num_control_points, spline_order
    )
...
def resample_points(points, spacing):
    ...
    interp_func_x = scipy.interpolate.interp1d(
        cumulative_distances, points[:, 0], kind="linear"
    )
    interp_func_y = scipy.interpolate.interp1d(
        cumulative_distances, points[:, 1], kind="linear"
    )
...
def find_ridge_line(
    cellAssign,
    ridgeNeighborIndecies,
    vertex1,
    vertex2,
    radarParams,
    radarParamsCovDeterminants,
    allVertecies,
    vertex1Index,
    vertex2Index,
    bounds,
    ...
    cellAssignment = np.where(cellAssign == i, 1, 0).reshape(
    ...
    contour, _ = cv2.findContours(
        cellAssignment.astype(np.uint8).T, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    pointsInRange = new_filter_points_by_vertices(
        points,
        vertex1,
        vertex2,
        centroid,
    ...
    #     cellAssign.reshape(params.numTestPoints, params.numTestPoints),
    # )
    # fig.colorbar(c, ax=ax)
    ...
    splineControlPoints, splineKnotPoints = fit_spline_to_path(
        pointsInRange, 10, 3, vertex1, vertex2
    )
...
def find_generalized_voronoi_ridges(
    ...
    for i in range(len(verticies)):
        for j in range(i + 1, len(verticies)):
            commonNeighbors = np.intersect1d(
                verticies[i]["neighbors"], verticies[j]["neighbors"]
            )
    ...
    for neighbors in potentialRidges.keys():
        ...
        if len(potentialRidges[neighbors]) == 4:
            ...
            _, indecies = sort_points(
                points, [params.bounds[0] / 2, params.bounds[1] / 2]
            )
            ignore.append(
                (
                    potentialRidges[neighbors][indecies[0]],
                    potentialRidges[neighbors][indecies[2]],
                )
            )
            ignore.append(
                (
                    potentialRidges[neighbors][indecies[0]],
                    potentialRidges[neighbors][indecies[3]],
                )
            )
            ignore.append(
                (
                    potentialRidges[neighbors][indecies[1]],
                    potentialRidges[neighbors][indecies[3]],
                )
            )
            ignore.append(
    ...
    for i in range(len(verticies)):
        for j in range(i + 1, len(verticies)):
            ...
            if len(commonNeighbors) == 3:
                ...
                ridgeLineControlPoints1, ridgeLineKnotPoints1, points1 = (
                    find_ridge_line(
                        prob_list,
                        commonNeighbors[0:2],
                        verticies[i]["point"],
                ...
                splinePoints1 = scipy.interpolate.BSpline(
                    ridgeLineKnotPoints1, ridgeLineControlPoints1, 3
                )(t)
                probPDLessThanThreshold1 = (
                    highPriorityHelperFunctions.get_prob_pd_less_than_threshold(
                        splinePoints1,
                        radarParams,
                        radarParamsCov,
                        params.probabilityOfDetectionThreshold,
                ...
                ridgeLineControlPoints2, ridgeLineKnotPoints2, points2 = (
                    find_ridge_line(
                        prob_list,
                        commonNeighbors[1:],
                        verticies[i]["point"],
                ...
                splinePoints2 = scipy.interpolate.BSpline(
                    ridgeLineKnotPoints2, ridgeLineControlPoints2, 3
                )(t)
                ...
                if minProb2 > thresholdConfidence and minProb1 > thresholdConfidence:
                    if minProb1 > minProb2:
                        ridges[(i, j)] = {
                            "control_points": ridgeLineControlPoints1,
                            "knot_points": ridgeLineKnotPoints1,
                        }
                    else:
                        ridges[(i, j)] = {
                            "control_points": ridgeLineControlPoints2,
                            "knot_points": ridgeLineKnotPoints2,
                        }
                elif minProb1 > thresholdConfidence:
                    ridges[(i, j)] = {
                        "control_points": ridgeLineControlPoints1,
                        "knot_points": ridgeLineKnotPoints1,
                    }
                elif minProb2 > thresholdConfidence:
                    ridges[(i, j)] = {
                        "control_points": ridgeLineControlPoints2,
                        "knot_points": ridgeLineKnotPoints2,
                    }
            ...
            if len(commonNeighbors) == 2:
                if (i, j) in ignore or (j, i) in ignore:
                    ...
                    ridgeLineControlPoints, ridgeLineKnotPoints, points = (
                        find_ridge_line(
                            prob_list,
                            commonNeighbors,
                            verticies[i]["point"],
                    ...
                    ###############
                    t = np.linspace(0, 1, 1000)
                    splinePoints = scipy.interpolate.BSpline(
                        ridgeLineKnotPoints, ridgeLineControlPoints, 3
                    )(t)
                    ...
                    if minProb > thresholdConfidence:
                        ridges[(i, j)] = {
                            "control_points": ridgeLineControlPoints,
                            "knot_points": ridgeLineKnotPoints,
                        }
...
def find_exterior_points(contourList, bounds):
    ...
    for i, points in enumerate(contourList):
        ...
        if (
            np.any(np.isclose(points[:, 0], 0, atol=tolerance))
            or np.any(np.isclose(points[:, 0], bounds[0], atol=tolerance))
            or np.any(np.isclose(points[:, 1], 0, atol=tolerance))
            or np.any(np.isclose(points[:, 1], bounds[1], atol=tolerance))
        ):
...
def find_edge_points(cellAssignment, bounds, numTestPoints):
    ...
    edgePoints = points[
        close_to_x_lower | close_to_x_upper | close_to_y_lower | close_to_y_upper
    ]
...
def find_edge_vertex(cellAssignment, i, j, verticies, vertexIndex, bounds):
    ...
    cellAssignmentI = np.where(cellAssignment == i, 1, 0).reshape(
        numTestPoints, numTestPoints
    )
    cellAssignmentJ = np.where(cellAssignment == j, 1, 0).reshape(
        numTestPoints, numTestPoints
    )
...
def find_possible_edge_vertex(prob_list, i, j, bounds, numTestPoints):
    ...
    cellAssignment = np.where(celliprob < celljprob, 0, 1).reshape(
        numTestPoints, numTestPoints
    )
...
def find_generalized_voronoi_edge_verticies(
    cellAssignments, exteriorPoints, verticies, points, bounds
):
    ...
    for i in range(len(exteriorPoints)):
        for j in range(i + 1, len(exteriorPoints)):
            ...
            if vertex is not None:
                verticies[currentVertex] = {
                    "neighbors": [exteriorPoints[i], exteriorPoints[j]],
                    "point": vertex,
                    "edge": True,
                }
                currentVertex += 1
...
def find_boundary_segments(
    verticies, radarParams, radarParamsCov, bounds, params, thresholdConfidence
):
    print("thresholdConfidence", thresholdConfidence)
    startVertexIndex = len(verticies)
    verticies[len(verticies)] = {
        "point": np.array([0, 0]),
        "edge": True,
        "neighbors": [],
        "type": "start",
    ...
    verticies[len(verticies)] = {
        "point": np.array([bounds[0], 0]),
        "edge": True,
        "neighbors": [],
    }
    verticies[len(verticies)] = {
        "point": np.array([0, bounds[1]]),
        "edge": True,
        "neighbors": [],
    }
    endVertexIndex = len(verticies)
    verticies[len(verticies)] = {
        "point": np.array([bounds[0], bounds[1]]),
        "edge": True,
        "neighbors": [],
        "type": "end",
    }
    ...
    edgeVerticies = np.array(
        [
            verticies[vertex]["point"]
            for vertex in verticies
            if verticies[vertex]["edge"]
        ]
    )
    edgeVerticiesIndicies = np.array(
        [vertex for vertex in verticies if verticies[vertex]["edge"]]
    )

    leftEdgeVertexIndicies = np.where(np.isclose(edgeVerticies[:, 0], 0, atol=50))[0]
    sortedLeftEdgeVertexIndicies = leftEdgeVertexIndicies[
        np.argsort(edgeVerticies[leftEdgeVertexIndicies][:, 1])
    ]
    rightEdgeVertexIndicies = np.where(
        np.isclose(edgeVerticies[:, 0], bounds[0], atol=50)
    )[0]
    sortedRightEdgeVertexIndicies = rightEdgeVertexIndicies[
        np.argsort(edgeVerticies[rightEdgeVertexIndicies][:, 1])
    ]
    topEdgeVertexIndicies = np.where(
        np.isclose(edgeVerticies[:, 1], bounds[1], atol=50)
    )[0]
    sortedTopEdgeVertexIndicies = topEdgeVertexIndicies[
        np.argsort(edgeVerticies[topEdgeVertexIndicies][:, 0])
    ]
    bottomEdgeVertexIndicies = np.where(np.isclose(edgeVerticies[:, 1], 0, atol=50))[0]
    sortedBottomEdgeVertexIndicies = bottomEdgeVertexIndicies[
        np.argsort(edgeVerticies[bottomEdgeVertexIndicies][:, 0])
    ]
    ...
    for i in range(len(sortedLeftEdgeVertexIndicies) - 1):
        ...
        boundarySegments = add_boundary_segment(
            boundarySegments,
            np.array(
                [
                    edgeVerticies[index],
                    edgeVerticies[sortedLeftEdgeVertexIndicies[i + 1]],
                ]
            ),
            edgeVerticiesIndicies[index],
            edgeVerticiesIndicies[sortedLeftEdgeVertexIndicies[i + 1]],
            radarParams,
            radarParamsCov,
            params,
            thresholdConfidence,
    ...
    for i in range(len(sortedRightEdgeVertexIndicies) - 1):
        ...
        boundarySegments = add_boundary_segment(
            ...
            np.array(
                [
                    edgeVerticies[index],
                    edgeVerticies[sortedRightEdgeVertexIndicies[i + 1]],
                ]
            ),
            edgeVerticiesIndicies[index],
            edgeVerticiesIndicies[sortedRightEdgeVertexIndicies[i + 1]],
            radarParams,
            radarParamsCov,
            params,
            thresholdConfidence,
    ...
    for i in range(len(sortedTopEdgeVertexIndicies) - 1):
        ...
        boundarySegments = add_boundary_segment(
            ...
            np.array(
                [
                    edgeVerticies[index],
                    edgeVerticies[sortedTopEdgeVertexIndicies[i + 1]],
                ]
            ),
            edgeVerticiesIndicies[index],
            edgeVerticiesIndicies[sortedTopEdgeVertexIndicies[i + 1]],
            radarParams,
            radarParamsCov,
            params,
            thresholdConfidence,
    ...
    for i in range(len(sortedBottomEdgeVertexIndicies) - 1):
        ...
        boundarySegments = add_boundary_segment(
            ...
            np.array(
                [
                    edgeVerticies[index],
                    edgeVerticies[sortedBottomEdgeVertexIndicies[i + 1]],
                ]
            ),
            edgeVerticiesIndicies[index],
            edgeVerticiesIndicies[sortedBottomEdgeVertexIndicies[i + 1]],
            radarParams,
            radarParamsCov,
            params,
            thresholdConfidence,
...
def find_generalized_voronoi(
    radarParams, radarParamsCov, bounds, params, thresholdConfidence
):
    ...
    prob_list = create_probability_of_detection_below_threshold_grid_list(
        X_test, radarParams, radarParamsCov, params
    )
    ...
    verticies, contourPoints, cellAssignments, radarIndeciesToIgnore = (
        find_generalized_voronoi_verticies(
            prob_list, bounds=params.bounds, numTestPoints=numTestPoints
        )
    )

    cellAssignments = remove_radar_from_cell_assignments(
        cellAssignments, radarIndeciesToIgnore
    )
    ...
    verticies = find_generalized_voronoi_edge_verticies(
        cellAssignments, exteriorPoints, verticies, points, bounds
    )
    boundarySegments, startVertexIndex, endVertexIndex = find_boundary_segments(
        verticies, radarParams, radarParamsCov, bounds, params, thresholdConfidence
    )
    ...
    ridges = find_generalized_voronoi_ridges(
        cellAssignments,
        verticies,
        radarParams,
        radarParamsCov,
...
def plot_generalized_voronoi(verticies, ridges, boundarySegments, ax):
    ...
    for vertex in verticies:
        ax.scatter(
            verticies[vertex]["point"][0],
            verticies[vertex]["point"][1],
            marker="*",
            color=c,
            s=100,
        )
    for segment in boundarySegments:
        ax.plot(
            boundarySegments[segment][:, 0],
            boundarySegments[segment][:, 1],
            c=c,
            linewidth=3,
        )
...
def create_adjacency_matrix_from_ridges_and_boundary(
    ridges, boundarySegments, verticies
):
    adjacencyMatrix = np.zeros((len(verticies), len(verticies)))

    for ridge in ridges.keys():
        spline = interpolate.BSpline(
            ridges[ridge]["knot_points"], ridges[ridge]["control_points"], 3
        )
    ...
    for segment in boundarySegments.keys():
        ...
        distance = np.linalg.norm(
            boundarySegments[segment][0] - boundarySegments[segment][1]
        )
...
def create_graph_and_find_shortest_path(adejacenyMatrix, startIndex, endIndex):
    g = igraph.Graph.Weighted_Adjacency(
        adejacenyMatrix.tolist(), mode=igraph.ADJ_UNDIRECTED, attr="weight"
    )
...
def path_to_points(path, ridges, boundarySegments, verticies, spacing):
    ...
    for i in range(len(path) - 1):
        ...
        numPoints = (
            int(
                np.linalg.norm(
                    verticies[path[i]]["point"] - verticies[path[i + 1]]["point"]
                )
                / spacing
            )
            + 1
        ...
        if ridgeVertex in ridges.keys():
            spline = interpolate.BSpline(
                ridges[ridgeVertex]["knot_points"],
                ridges[ridgeVertex]["control_points"],
                3,
            )
            points = spline(np.linspace(0, 1, numPoints))
        elif ridgeVertex in boundarySegments.keys():
            points = np.linspace(
                verticies[path[i]]["point"], verticies[path[i + 1]]["point"], numPoints
            )
        elif flippedRidgeVertex in ridges.keys():
            spline = interpolate.BSpline(
                ridges[flippedRidgeVertex]["knot_points"],
                ridges[flippedRidgeVertex]["control_points"],
                3,
            )
            points = spline(np.linspace(0, 1, numPoints))
        elif flippedRidgeVertex in boundarySegments.keys():
            points = np.linspace(
                verticies[path[i + 1]]["point"], verticies[path[i]]["point"], numPoints
            )
...
def find_initial_trajectory_uncertain_radar(
    radarParams, radarParamsCov, spacing, bounds, params, ax=None
):
    # fig,ax = plt.subplots()
    start = time()
    (
        ...
        endVertexIndex,
        cellAssinments,
    ) = find_generalized_voronoi(
        radarParams, radarParamsCov, bounds, params, params.thresholdConfidence
    )
    ...
    #     params.X_test[:, 0].reshape(params.numTestPoints, params.numTestPoints),
    #     params.X_test[:, 1].reshape(params.numTestPoints, params.numTestPoints),
    #     cellAssinments.reshape(params.numTestPoints, params.numTestPoints),
    # )
    # plot_generalized_voronoi(verticies, ridges, boundarySegments, ax)
    # plt.show()
    print("Time", time() - start)

    adjecencyMatrix = create_adjacency_matrix_from_ridges_and_boundary(
        ridges, boundarySegments, verticies
    )
    path = create_graph_and_find_shortest_path(
        adjecencyMatrix, startVertexIndex, endVertexIndex
    )
...
def main():
    ...
    radarParams, radarParamsCov = load_estimated_params(
        dataFilePath, dataIndex, len(radarList)
    )

    Z = safe_corridors_uncertain_radar(
        params.X_test,
        params.probabilityOfDetectionThreshold,
    ...
    pathPoints = find_initial_trajectory_uncertain_radar(
        radarParams, radarParamsCov, 100, bounds=params.bounds, params=params, ax=ax
    )
