...
class Agent:
    def __init__(
        ...
        numRadar,
        sensingRange,
        powerMeasurementStdDev,
        angleMeasurementStdDev,
        elintAntenneaGain,
        elintSystemLoss,
        emittorWavelength,
        radarCrossSection,
        ...
        self.radarMeasurementCoeff = (
            self.elintAntenneaGain * self.emittor_signal_wavelength**2
        ) / ((4 * np.pi) ** 2 * self.elsintSystemLoss)

        self.plotCircle = Circle(
        ...
        self.plotLine = Line2D(
            xdata=[self.position[0], x_end],
            ydata=[self.position[1], y_end],
            linewidth=3,
            color=plotColor,
            zorder=100000000,
        )
    ...
    def measure_radar(self, radar_list, measurementNumber):
        ...
        for i, radar in enumerate(radar_list):
            ...
            if dist < self.sensingRange:
                angle_between_radar_and_agent = np.arctan2(
                    self.position[1] - radar.position[1],
                    self.position[0] - radar.position[0],
                )
                ...
                if (
                    angle_between_radar_and_agent < radar_angle + radar.beamwidth / 2
                    and angle_between_radar_and_agent
                    > radar_angle - radar.beamwidth / 2
                ):
                    ...
                    power_measurements.append(
                        ...
                        / dist**2
                    )  # + np.random.normal(0,self.powerMeasurementStdDev)**2
                    # aoa measured in global frame
                    angle_of_arrivals.append(
                        self.map_angle_minus_pi_to_pi(
                            angle_between_radar_and_agent
                            + np.pi
                            + self.params.rng.normal(0, self.angelMeasurementStdDev)
                        )
                    )
        ...
        if len(angle_of_arrivals) == 1:
            ...
            self.measurementLocations.append(self.position[0:2])
            self.measurementAngleOfArrivalValues.append(
                angle_of_arrivals[closest_radar_index]
            )
            # self.truthEmitterCorrespondence[radar_indecies[closest_radar_index]].append(len(self.measurementLocations)-1)
            self.truthEmitterCorrespondence[radar_indecies[closest_radar_index]].append(
                measurementNumber
            )
    ...
    def plot_power_measurements(self, ax):
        if len(self.measurementLocations) > 0:
            ...
            p = ax.scatter(
                data[:, 0],
                data[:, 1],
                c=np.log10(np.array(self.measurementPowerValues)),
                vmin=-5,
                vmax=0.5,
            )
