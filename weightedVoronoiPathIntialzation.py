...
def compute_bisector_points_to_point(points, weights, point, weight):
    ...
    for i in range(len(points)):
        center, radius = compute_bisector_two_points(
            points[i], point, weights[i], weight
        )
...
def intersect_arcs_with_boundary(arcs, bounds, ax):
    ...
    for i, arc in enumerate(arcs):
        ...
        if plot:
            ...
            plot_arc(
                arc[4:6],
                np.linalg.norm(arc[0:2] - arc[4:6]),
                theta1,
                theta2,
                ax=ax,
                c="b",
...
def create_adjacency_matrix_from_arcs_and_bounary(
    arcs, boundarySegments, bounds, ax=None
):
    nodes = dict()
    currentNodeNumber = 0
...
def create_graph_and_find_shortest_path(adejacenyMatrix, nodes):
    g = ig.Graph.Weighted_Adjacency(
        adejacenyMatrix.tolist(), mode=ig.ADJ_UNDIRECTED, attr="weight"
    )
...
def fill_in_path(path, edges, nodes, spacing=500):
    ...
    for i in range(len(path[0]) - 1):
        if edges[(path[0][i], path[0][i + 1])]["arc"]:
            ...
            if edges[(path[0][i], path[0][i + 1])]["direction"] == "counterClockwise":
                arc = evaluate_arc(
                    center, np.linalg.norm(p1 - center), theta1, theta2, spacing
                )
            else:
                arc = evaluate_arc(
                    center, np.linalg.norm(p1 - center), theta2, theta1, spacing
                )
            # arc = evaluate_arc(center,np.linalg.norm(p1-center),theta1,theta2,spacing)
            newPath.append(arc)
        else:
            numPoints = int(
                np.linalg.norm(nodes[path[0][i]] - nodes[path[0][i + 1]]) / spacing
            )
...
def dist_points_to_arc(arc, points, weights, ax=None):
    ...
    if np.any(inRange):
        intersectionPoints = (
            center
            + radius * np.array([np.cos(thetas[inRange]), np.sin(thetas[inRange])]).T
        )
        distances[inRange] = np.divide(
            np.linalg.norm(intersectionPoints - points[inRange], axis=1),
            weights[inRange],
        )
    ...
    if np.any(~inRange):
        intersectionPointsAll[~inRange] = np.array(
            [
                arc[0:2].reshape((2, 1)) if a else arc[2:4].reshape((2, 1))
                for a in dist_A[~inRange] < dist_B[~inRange]
            ]
        ).squeeze()
...
def trim_arcs_pd_threshold(arcs, radarList, weights, pdThreshold, params, ax):
    ...
    for a in range(len(arcs) - 1, -1, -1):
        ...
        arc = arcs[a]
        distances, intersecionts, inRange = dist_points_to_arc(
            arc, radarPositions, weights
        )
        pdAtIntersections = ground_truth_probability_of_detection(
            jnp.array(intersecionts),
            tuple(radarList),
            params.radarWavelengthPriorMean,
            params.agentRadarCrossSection,
            params.radarPulseWidth,
        ...
        if plot:
            if pdAtIntersections[maxPdIndex] > pdThreshold:
                ...
                for i, inter in enumerate(intersecionts):
                    ...
                    ax1.plot(
                        [radarPositions[i][0], arc[4]],
                        [radarPositions[i][1], arc[5]],
                        c="k",
                        linestyle="--",
                    )
                    ax1.plot(
                        [radarPositions[i][0], inter[0]],
                        [radarPositions[i][1], inter[1]],
                        c="c",
                    )
                plt.show()
    return arcs
...
def remove_unfeasible_segments(segments, radarPositions, params):
    minRadarIndicies = np.array(
        [
            np.argmin(dist_of_points_to_line_segment(seg[0], seg[1], radarPositions))
            for seg in segments
        ]
    )
    minRadarDists = np.array(
        [
            np.min(dist_of_points_to_line_segment(seg[0], seg[1], radarPositions))
            for seg in segments
        ]
    )
    ...
    if plot:
        ...
        for seg in segments[
            ~np.greater(minRadarDists, params.safePdDists[minRadarIndicies])
        ]:
            # for seg in segments:
            fig, ax = plt.subplots()
            ...
            for i, rad in enumerate(radarPos):
                circle = plt.Circle(
                    rad, params.safePdDists[minRadarIndicies[i]], fill=False
                )
...
def compute_path_weighted_voronoi(radarList, params, plot=False, ax=None):
    radarPositions, weights = save_points_and_weights_to_file(
        radarList, "my_input.pnts"
    )
    ...
    adjacencyMatrix, nodes, edges = create_adjacency_matrix_from_arcs_and_bounary(
        arcs, boundarySegments, params.bounds, ax
    )
